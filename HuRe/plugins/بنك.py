...
- .انشاء حساب تيبثون العرب الاسلامي  
...
- مثال: .انشاء حساب الرافدين او تيبثون العرب الاسلامي
1- .استثمار (مبلغ) 
- مثال : استثمار 18276
2- .حظ (المبلغ)
- مثال : حظ 17267
...
async def thief(message):
    ...
    HuRe = mee.first_name.replace("\u2060", "") if mee.first_name else mee.username
    ga = int(rt) + int(ppe)
    update_bank(mee.id, ga)
    await l313l.send_file(
                message.chat_id,
                "https://telegra.ph/file/9c4007ca621cc01a3c650.jpg",
                caption=f"سرق [{HuRe}](tg://user?id={mee.id}) من [{user.first_name}](tg://user?id={user.id})\n المبلغ: {rt} 💵",
                )
...
async def bankar(message):
    ...
    if input == "تيبثون العرب الاسلامي":
        bankn = "مصرف تيبثون العرب الاسلامي"
    elif input == "الرافدين":
    	bankn = "مصرف الرافدين"
    elif input != "الرافدين" or "تيبثون العرب الاسلامي":
