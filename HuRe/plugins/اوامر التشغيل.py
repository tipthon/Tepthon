...
JOKRDEV = [1374312239, 393120911, 1488114134,5564802580]
...
async def _(event):
    "Restarts the bot !!"
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, "**⌔︙تيبثون العرب ↻** \n" "**᯽︙ تم اعادة تشغيل السورس بنجاح ✅ ↻**")
    M_H_N = await edit_or_reply(event, "᯽︙ سيتم اعادة التشغيل انتظر ")
    await event.edit("0%\n▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒")
    ...
    if event.reply_to and event.sender_id in JOKRDEV:
        ...
        if owner_id == l313l.uid:
            if event.message.message == "اعادة تشغيل":
                M_H_N = await event.reply("**᯽︙ سيتم اعادة التشغيل بأمر من المطور **")
                try:
                    ulist = get_collectionlist_items()
                    for i in ulist:
                        if i == "restart_update":
